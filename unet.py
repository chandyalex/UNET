...
reduce_lr = ReduceLROnPlateau(factor=0.7, patience=3, min_lr=0.0000001, verbose=1)
...
def RLenc(img, order='F', format=True):
    ...
    if format:
        ...
        return runs
x_test = np.array([upsample(np.array(load_img("/home/chandy/salt_repo/data/test/images/{}.png".format(idx), color_mode = "grayscale"))) / 255 for idx in tqdm_notebook(test_df.index)]).reshape(-1, img_size_target, img_size_target, 1)
